...
v_pec = np.linspace(0, 1000, 100)
Z, V = np.meshgrid(z, v_pec)

# 2. Parámetros físicos reales de la Teoría Q.A.S.T.
V_rot = 240.0      # Escala de acoplamiento cinemático de la misión Gaia (km/s)
z_decay = 0.01     # Parámetro de decaimiento dinámico de la burbuja local (~30 Mpc)
...
H0_surface = H0_baseline + 2.3 * np.log10(1 + actividad) * np.exp(-Z / z_decay)
...
fig = go.Figure(data=[go.Surface(
    ...
    y=V, 
    colorscale='Viridis',
    colorbar=dict(title='H0 Aparente (km/s/Mpc)')
)])

# 5. Configuración de los ejes y entorno visual (Estilo Dark/Académico)
fig.update_layout(
    title='Superficie de Solución Q.A.S.T.: H0 vs. Redshift y Velocidad Peculiar',
    scene=dict(
